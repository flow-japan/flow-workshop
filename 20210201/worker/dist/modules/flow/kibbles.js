    ...
    constructor(flowService, fungibleTokenAddress, kibbleAddress) {
        ...
        this.setupAccount = async () => {
            ...
            const transaction = fs
                .readFileSync(path.join(process.env.CADENCE_PATH || __dirname, `../../cadence/kibble/transactions/setup_account.cdc`), 'utf8')
        ...
        this.mint = async (recipient, amount) => {
            const authorization = this.flowService.authorizeMinter();
            const transaction = fs
                .readFileSync(path.join(process.env.CADENCE_PATH || __dirname, `../../cadence/kibble/transactions/mint_tokens.cdc`), 'utf8')
                .replace(/0xFUNGIBLETOKENADDRESS/gi, `0x${this.fungibleTokenAddress}`)
                .replace(/0xKIBBLE/gi, `0x${this.kibbleAddress}`);
            return this.flowService.sendTx({
                transaction,
        ...
        this.burn = async (amount) => {
            const authorization = this.flowService.authorizeMinter();
            const transaction = fs
                .readFileSync(path.join(process.env.CADENCE_PATH || __dirname, `../../cadence/kibble/transactions/burn_tokens.cdc`), 'utf8')
                .replace(/0xFUNGIBLETOKENADDRESS/gi, `0x${this.fungibleTokenAddress}`)
                .replace(/0xKIBBLE/gi, `0x${this.kibbleAddress}`);
            return this.flowService.sendTx({
                transaction,
        ...
        this.getBalance = async (account) => {
            const script = fs
                .readFileSync(path.join(process.env.CADENCE_PATH || __dirname, `../../cadence/kibble/scripts/get_balance.cdc`), 'utf8')
                .replace(/0xFUNGIBLETOKENADDRESS/gi, `0x${this.fungibleTokenAddress}`)
                .replace(/0xKIBBLE/gi, `0x${this.kibbleAddress}`);
        ...
        this.getSupply = async () => {
            const script = fs
                .readFileSync(path.join(process.env.CADENCE_PATH || __dirname, `../../cadence/kibble/scripts/get_supply.cdc`), 'utf8')
                .replace(/0xFUNGIBLETOKENADDRESS/gi, `0x${this.fungibleTokenAddress}`)
                .replace(/0xKIBBLE/gi, `0x${this.kibbleAddress}`);
